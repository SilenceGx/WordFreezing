...
## 2026-08-02 — 修复：词本分页超宽溢出（大词本全量渲染）

### 背景
词本总览页分页在总页数较多时溢出容器：3612 词的词本（每页 50 条 = 73 页），模板 `range(1, total_pages + 1)` 渲染全部页码，flex 不换行直接超出页面宽度。

### 已修复
- [x] `wordbook_detail` 路由新增分页压缩：只显示当前页前后各 2 页 + 首页尾页，缺口用 `None` 标记
- [x] `wordbook.html` 模板改用 `page_list`，`None` 渲染省略号（`…`）
- [x] `.pagination` 增加 `flex-wrap: wrap` + `max-width: 100%` 兜底防溢出，新增 `.page-ellipsis` 样式

### 验证
- 73 页词本：首页渲染 `1 2 3 … 73`，末页 `1 … 71 72 73`，均 ≤7 个元素居中不溢出
- 小词本（≤5 页）行为不变
