...
## 2026-08-04 — 修复：DeepSeek 瞬时 503 自动重试

### 背景
调用 AI 时偶发 `AI 评判失败: API 请求失败: 503 Server Error: Service Temporarily Unavailable`。

排查结论：**非配置问题**。经实测 API Key 有效、`deepseek-v4-flash` 模型存在、请求体（含 `thinking: disabled`）正常，503 是 DeepSeek 服务端瞬时不可用（高峰期常见），此前代码不重试，单次失败即报错。

同时澄清：URL 里的 `/v1` 是 OpenAI 兼容 API 版本前缀，**不是模型版本**；模型版本在请求体 `model` 字段（已是 `deepseek-v4-flash`），无需改动。

### 已修复
- [x] `_call_ai` DeepSeek 分支加入瞬时错误自动重试：429/500/502/503/504 指数退避（1s→2s），最多重试 2 次
- [x] 重试耗尽后返回友好文案「AI 服务暂时不可用 (HTTP 503)，已自动重试 2 次仍失败，请稍后再试」
- [x] 400（模型名非法）保持不重试、直接提示；网络层错误逻辑不变

### 版本
- [x] main 提交 `6a15b15`，打 tag `v2.1.2`
